...
<cfcomponent
	...
	<cffunction name="ReadExcel" access="public" returntype="any" output="false"
		...
		<cfscript>
			...
			var local.jarPaths = arrayNew(1);

			arrayAppend(local.jarPaths, expandPath('{railo-server}../../apache-poi.jar'));

			LOCAL.javaLoader = createObject("component","mandel.javaloader.javaLoader").init(local.jarPaths);
			...
			LOCAL.ExcelFileSystem = LOCAL.javaLoader.create("org.apache.poi.poifs.filesystem.POIFSFileSystem").Init(LOCAL.FileInputStream );

			LOCAL.WorkBook = LOCAL.javaLoader.create("org.apache.poi.hssf.usermodel.HSSFWorkbook").Init(LOCAL.ExcelFileSystem);
